...
public class PhotoDAO {
	...
	public PhotoDAO() throws DAOException {
		try {
	        // Get a BeanTable to create the "photo" table
	        BeanTable<Photo> photoTable = BeanTable.getInstance(Photo.class,"photo");
	...
	public void delete(int id, String owner) throws DAOException {
		try {
    		...
    		if (!owner.equals(p.getOwner())) {
				throw new DAOException("Photo not owned by "+owner);
    		}
	...
	public Photo[] getPhotos(String owner) throws DAOException {
		try {
	...
	public Photo[] moveDown(int id, String owner) throws DAOException {
		try {
			Transaction.begin();
			Photo[] list = factory.match(MatchArg.equals("owner",owner));
			Arrays.sort(list);
			...
			if (index == -1) throw new DAOException("Photo not owned by "+owner);
	...
	public Photo[] moveUp(int id, String owner) throws DAOException {
		try {
			Transaction.begin();
			Photo[] list = factory.match(MatchArg.equals("owner",owner));
			Arrays.sort(list);
